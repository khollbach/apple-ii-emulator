...
fn sheet_to_dots(sheet: Vec<Vec<u8>>) -> Vec<Vec<Color>> {
    sheet.into_iter().map(line_of_dots).collect()
}

fn line_of_dots(row: Vec<u8>) -> Vec<Color> {
    row.into_iter().flat_map(byte_to_dots).collect()
}

fn byte_to_dots(byte: u8) -> [Color; 7] {
    // ignore hibit, for now

    let mut out = [Color::Black; 7];
    for i in 0..7 {
        let is_set = byte & 1 << i != 0;
        out[i] = if is_set { Color::White } else { Color::Black };
    }
    out
}

fn weave_sheets(sheets: [Vec<Vec<Color>>; 8]) -> Vec<Vec<Color>> {
